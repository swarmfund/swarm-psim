...
func TestWithFields_GetFields(t *testing.T) {
	...
	tests := []struct {
		...
		name: "fields.From",
		err:  errors.From(errors.New("foo"), f1),
		out:  f1,
	}, {
		name: "fields.Wrap without fields",
		err:  errors.Wrap(errors.New("foo"), "bar"),
		out:  map[string]interface{}{},
	}, {
		name: "fields.Wrap with single fields",
		err:  errors.Wrap(errors.New("foo"), "bar", f1),
		out:  f1,
	}, {
		name: "fields.Wrap with multiple fields",
		err:  errors.Wrap(errors.New("foo"), "bar", f1, f2),
		out:  f1.AddFields(f2),
	}, {
		name: "getting transitive fields",
		err:  errors.Wrap(errors.From(errors.New("foo"), f1), "bar", f2),
		out:  f1.AddFields(f2),
	}, {
		name: "getting transitive fields with non-fielded mediator",
		err:  errors.Wrap(errors.Wrap(errors.From(errors.New("foo"), f1), "bar"), "bar2", f2),
		out:  f1.AddFields(f2),
	},
	}
