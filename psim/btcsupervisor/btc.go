...
func (s *Service) processBlock(ctx context.Context, blockIndex uint64) error {
	...
	for _, tx := range block.Txs {
		...
		err := s.processTX(ctx, blockHash, time.Unix(int64(block.MedianPastTime), 0), *tx)
		if err != nil {
...
func (s *Service) processTX(ctx context.Context, blockHash string, blockTime time.Time, tx btc.Tx) error {
	for i, out := range tx.TxOut {
		...
		// TODO Add conversion from BTC to the system Asset using Prices.
		// 10^8 / 10^6
		var div uint64 = 100
		convertedAmount := out.Value / div
		...
		err := s.sendCoinEmissionRequest(ctx, blockHash, txHash, i, *accountAddress, convertedAmount)
		if err != nil {
			return errors.Wrap(err, "Failed to send CoinEmissionRequest",
				logan.Field("account_address", *accountAddress).
					...
					Add("converted_amount", convertedAmount),
			)
		}
	}
